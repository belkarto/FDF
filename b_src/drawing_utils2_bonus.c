...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   drawing_utils2_bonus.c                             :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: belkarto <marvin@42.fr>                    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/01/31 11:00:00 by belkarto          #+#    #+#             */
/*   Updated: 2023/02/15 18:23:50 by belkarto         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/* void	normal(t_fdf *fdf)
{
	...
	while (++i < fdf->map_high)
	{
		...
		while (++j < fdf->map_width)
		{
			fdf->pnt_af[i][j].p_x = fdf->pnt[i][j].p_x;
			fdf->pnt_af[i][j].p_y = fdf->pnt[i][j].p_y;
			fdf->pnt_af[i][j].p_x *= fdf->step;
			fdf->pnt_af[i][j].p_y *= fdf->step;
			fdf->pnt_af[i][j].p_color = fdf->pnt[i][j].p_color;
			fdf->pnt_af[i][j].h = fdf->pnt[i][j].h;
		}
	}
} */
